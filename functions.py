...
def minibatch_kmeans(rdd_train, k: int, b: int, epochs: int, seed: int, initial_centers=None):
    ...
    fraction = min(1.0, float(b)/total_count)
...
def b_search(rdd_train_files, rdd_test, initial_centers, K, b_list, epochs, num_iter, raw_csv, stats_csv, s3_creds=None):
    """Grid Search to find best b parameter among list of values."""
    start = time.time()
    results = []

    rdd_train = rdd_train_files
    rdd_train.persist()
    ...
    for run_id in range(num_iter):
        ...
        for b in b_list:
            ...
            centers = minibatch_kmeans(rdd_train, K, b, epochs, seed=run_id, initial_centers=initial_centers)
    ...
    rdd_train.unpersist()
    # Note: rdd_test is managed by the external driver, avoiding unpersist here.
