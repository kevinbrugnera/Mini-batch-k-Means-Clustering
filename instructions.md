...
In order to implement and use our workflow for the parallelization of K-Means clustering, you should strictly follow this instructions and use them whenever you have doubts. We tried our best to develop a framework that can be used by anyone using a Linux OS, enforcing consistency and setting up safe enviroments to run the scripts. 

**WARNING: Everything should be run inside the current `Project/` directory you downloaded.**
...
**2.** Run `python data_generation.py`.
...
**3.** Run `./run_script.sh grid_search.py` command.
